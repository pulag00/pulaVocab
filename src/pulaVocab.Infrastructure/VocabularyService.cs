...
public sealed class VocabularyService(VocabMasterDbContext db) : IVocabularyService
{
    ...
    public async Task<VocabularyEntryResponse?> FindByTermAsync(string term, Language language, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(term)) return null;
        var normalized = NormalizeTerm(term);
        var entry = await DetailsQuery().AsNoTracking().FirstOrDefaultAsync(x => x.Language == language && x.NormalizedTerm == normalized, cancellationToken);
    ...
    public async Task<VocabularyEntryResponse> CreateAsync(CreateVocabularyEntryRequest request, CancellationToken cancellationToken)
    {
        ...
        entry.Update(request.Term, request.Language, request.PartOfSpeech, request.Level, request.Status, request.Pronunciation, request.Ipa, request.IpaAmerican, request.IpaBritish, request.Synonyms, request.Antonyms, request.RelatedTerms, request.PersonalNotes, request.Source);
    ...
    {
        var normalized = NormalizeTerm(term);
        if (await db.VocabularyEntries.AnyAsync(x => x.Language == language && x.NormalizedTerm == normalized && (!ignoredId.HasValue || x.Id != ignoredId), ct))
        {
            throw new InvalidOperationException("Ya existe una palabra con ese término en el idioma seleccionado.");
        }
    }
    private static void Validate(CreateVocabularyEntryRequest request)
    {
        ...
        if (request.Term.Length > 200) throw new ArgumentException("El término no puede exceder 200 caracteres.");
        ...
        if (request.PartOfSpeech == PartOfSpeech.Verb && request.EnglishDetails is { } english && request.Language == Language.English)
        {
            // No mandatory conjugations, but normalize if provided.
        }
    }
    private static void SetDetails(VocabularyEntry entry, CreateVocabularyEntryRequest request)
    {
        if (request.EnglishDetails is { } e) entry.SetEnglishDetails(new EnglishWordDetails(entry.Id) { Infinitive = e.Infinitive?.Trim(), PastTense = e.PastTense?.Trim(), PastParticiple = e.PastParticiple?.Trim(), ThirdPersonSingular = e.ThirdPersonSingular?.Trim(), Gerund = e.Gerund?.Trim(), IsIrregularVerb = e.IsIrregularVerb, RelatedPhrasalVerbs = e.RelatedPhrasalVerbs?.Trim() });
