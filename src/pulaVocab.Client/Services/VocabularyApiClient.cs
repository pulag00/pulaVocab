...
public sealed class VocabularyApiClient(HttpClient http) : IVocabularyApiClient
{
    ...
    private async Task<T> Send<TRequest, T>(HttpMethod method, string uri, TRequest body, CancellationToken ct) { var r = await http.SendAsync(new HttpRequestMessage(method, uri) { Content = JsonContent.Create(body) }, ct); return await Read<T>(r, ct); }
    private static async Task<T> Read<T>(HttpResponseMessage r, CancellationToken ct) { if (!r.IsSuccessStatusCode) throw new InvalidOperationException("No se pudo completar la operación."); return (await r.Content.ReadFromJsonAsync<T>(ct))!; }
}
